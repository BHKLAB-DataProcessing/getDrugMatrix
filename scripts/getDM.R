...
output_dir <- args[1]
processed_dir <- file.path(args[1], 'processed')
data_dir <- file.path(args[1], 'data')
output_dir <- "/Users/minoru/Code/bhklab/DataProcessing/ToxicoSet/getDrugMatrix/"
data_dir <- "/Users/minoru/Code/bhklab/DataProcessing/ToxicoSet/getDrugMatrix/data"
processed_dir <- "/Users/minoru/Code/bhklab/DataProcessing/ToxicoSet/getDrugMatrix/processed"

#reading the metadata downloaded from diXa
s_Hepatocyte <- read.delim(file.path(data_dir, "s_Hepatocyte.csv"), stringsAsFactors = F, sep = "\t")
...
curationDrug <- readRDS(file.path(data_dir, "curationDrug.rds"))
...
doselevel_mapping <- readRDS(file.path(data_dir, "doselevel_mapping.rds"))
...
create_exprsdata_DM <- function(species=c("Rat"), phenodata_DM, verbose = TRUE) {
  ...
  eset <- readRDS(file.path(processed_dir, "eset_DM.rds"))
...
saveRDS(drugMatrix, file.path(output_dir, "drugMatrix.rds"))
